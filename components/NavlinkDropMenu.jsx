      ...
      <Menu as="div" className="relative block text-left md:hidden">
        {({ open }) => (
          <div>
            ...
            >
              <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-zinc-400 rounded-md border border-zinc-400 bg-gray-100 shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none dark:divide-zinc-700 dark:border-zinc-700 dark:bg-zinc-800">
                <div className="py-1 ">
                  <div className="px-3 py-1 font-mplus text-xs font-bold uppercase">
                    Navigation
                  </div>
                </div>
                {NavLinks.filter((links) => links.title !== "").map((links) => (
                  <Menu.Item key={links.link}>
                    {({ active }) => (
                      <Link
                        href={links.link}
                        className={`block rounded-[3px] px-3 py-2 text-base ${
